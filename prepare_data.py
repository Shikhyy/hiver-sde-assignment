...
def prepare_dataset(csv_path, brand_author_id='AppleSupport', sample_size=250):
    ...
    df['in_response_to_tweet_id_str'] = df['in_response_to_tweet_id'].astype(str).str.replace(r'\.0$', '', regex=True)
    ...
    customer_inbounds_renamed = customer_inbounds.rename(columns={
        'tweet_id_str': 'customer_tweet_id',
        'text': 'customer_text',
        'author_id': 'customer_author_id'
    })[['customer_tweet_id', 'customer_text', 'customer_author_id']]
    
    brand_replies_renamed = brand_replies.rename(columns={
        'in_response_to_tweet_id_str': 'customer_tweet_id',
        'text': 'brand_response_text'
    })
    
    merged = pd.merge(customer_inbounds_renamed, brand_replies_renamed, on='customer_tweet_id', how='inner')
    ...
    golden_set['customer_text'] = golden_set['customer_text'].str.replace(f'@{brand_author_id} ', '', case=False)
