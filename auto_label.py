...
class Label(BaseModel):
    ...
    intent: str = Field(
        description="One of: technical_issue, account_access, repair_and_service, complaint, general_inquiry"
    )
    action: str = Field(
        description="One of: auto, escalate. Escalate if the user is extremely angry, threatening legal action, or requires sensitive account info. Otherwise auto."
    )
...
def auto_label():
    ...
    for i in range(0, len(df), chunk_size):
        ...
        print(
            f"Processing chunk {i} to {i+len(chunk)} (out of {len(df)})...", flush=True
        )
        ...
        try:
            ...
            for lbl in batch_result.get("labels", []):
                ...
                idx = df.index[
                    df["customer_tweet_id"].astype(str) == lbl["tweet_id"]
                ].tolist()
                if idx:
